...
class DAL {
    ...
    boolean apagar(int id) {
        ...
        // A cláusula where para o apagar. Note a interrogação. É um "wildcard".
        // Seu valor será inserido pelo contido na variável args
        String where = "_id = ?";
        String[] args = { String.valueOf(id) };
        ...
        // efetivamente faz o apagar no banco, fechando o acesso em seguida
        result = db.delete(CreateDatabase.TABLE, where, args);
        db.close();

        // Reporta um erro caso tenha acontecido
        if (result == -1) {
            Log.e(TAG, "insert: Deletando registro");
            return false;
        }
